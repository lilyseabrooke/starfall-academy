...
export function buildSpells(draft: Draft, D: ForgeData): SerializedSheet["magic"]["spells"] {
  ...
  return draft.spells
    .map((id) => {
      ...
      return { id: "sp-start-" + e.id, name: e.name, level: e.level, subjectKey: e.subjectKey || "", subject: e.subject || "", school: e.school || "", stat: e.stat || "", ap: e.ap, dc: e.dc ?? null, ritual: !!e.ritual, volatile: !!e.volatile, days: 0, desc: e.desc, higherLevel: e.higherLevel };
    })
    .filter((x): x is NonNullable<typeof x> => !!x);
}
