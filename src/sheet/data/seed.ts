/* ===========================================================================
   ...
   ---------------------------------------------------------------------------
   Ported from public/character-sheet/data.js (window.SF_DATA) to a typed ES
   module. This is the offline/standalone seed + reference data; the live
   Compendium (src/sheet/data/compendium.ts) overlays the DB rows on top of the
   `compendium` / class arrays here, falling back to these when a tab is
   unreachable.
   =========================================================================== */
import type {
  Bonus,
...
export interface CreationYear {
  ...
  label: string;
  roman: string;
  quick: { stat: number; subject: number; skill: number };
  custom: number;
  limit: number;
  spells: { Basic: number; Standard: number; Advanced: number };
}

export interface StartingWand {
  id: string;
  name: string;
  kind: "stat" | "ability";
  value: number;
...
export interface CreationRules {
  ...
  yields: {
    alchemyPotionMax: number;
    runologyGlyphsPerRank: number;
    wandcraftMatPerRank: number;
    herbalismPlantMat: number;
  };
  startingWands: StartingWand[];
}
...
/** A roll in the demo "conjure" pools / opening ledger. `hl` is a higher-level
 *  text function; the rest feed makeRoll(). */
export interface PoolRoll {
  ...
  success?: string | null;
  fail?: string | null;
  hl?: (deg: number, s: boolean) => string;
  dice?: number[];
  sit?: number;
  sitReason?: string;
}
...
export const SEED: SeedData = {
  ...
  creation: {
    ...
    yields: {
      alchemyPotionMax: 6,
      runologyGlyphsPerRank: 2,
      wandcraftMatPerRank: 200,
  ...
  compendium: [
    { id: "augurs-glimpse", cat: "spell", name: "Augur's Glimpse", tone: "plum", level: "Standard",
      meta: ["Divination", "DC 18", "Ritual"], cost: "50 mat.",
    ...
    { id: "hollow-lantern", cat: "spell", name: "Hollow Lantern", tone: "plum", level: "Advanced",
      meta: ["Necromancy", "DC 22"], cost: "200 mat.",
    ...
    { id: "kindle-ward", cat: "spell", name: "Kindle the Hearth-Ward", tone: "crimson", level: "Standard",
      meta: ["Evocation", "DC 16"], cost: "30 mat.",
    ...
    { id: "whispering-step", cat: "spell", name: "Whispering Step", tone: "teal", level: "Advanced",
      meta: ["Teleportation", "DC 20"], cost: "120 mat.",
    ...
    { id: "mending-cantrip", cat: "spell", name: "Mending Cantrip", tone: "forest", level: "Basic",
      meta: ["Restoration", "DC 12"], cost: "10 mat.",
