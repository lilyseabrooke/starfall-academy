/* ===========================================================================
   ...
   Plain JS on window.SF_INV. Read by inventory.jsx and the mount script.

   The character's *owned* inventory lives here. Linked Moves (on artifacts)
   and linked Bonuses (on wands) are defined inline; attuning an artifact or
   equipping a wand surfaces them on the Overview, and a status change on the
   item flows back to the linked Move.
   =========================================================================== */
(function () {
  window.SF_INV = {
    ...
    // Attunement degrees-of-failure → how much the intensity eases for next try
    // (a failed attempt loosens the artifact, mirroring the Roll20 sheet).
    attuneEase: { "-1": -5, "-2": -3, "-3": -3, "-4": -2, "-5": -2, "-6": -1, "-7": -1, "-8": -1, "-9": -1, "-10": 0, "-11": 1 },

    // ---- Artifact repair ------------------------------------------------
    // Any artifact whose status isn't Stable may be mended with an Artificy
    // roll. The faster the pace the higher the DC; a worse status raises it
    // further (Broken is harder than Damaged). Single source of truth for the
    // card UI and the roll handler.
    repair: {
    ...
    // ---- Artifacts — each carries an activation roll (Skill[s] · DC) ------
    // that surfaces a linked Move on your Overview once attuned. Skill(s) and
    // DC are pulled from the Artifacts database; the Move is the boon it grants.
    artifacts: [
    ...
    // ---- Held potions (the 6-cap tray of actual vials) ------------------
    potions: [
    ...
    // ---- Potion recipes (unlimited library you can brew from) -----------
    recipes: [
    ...
    // ---- Plants (sum of Value must stay ≤ 50 × Herbalism) ---------------
    plants: [
    ...
    // ---- Wands (any number; only one equipped) -------------------------
    // A wand isn't only a Bonus engine. Each carries a single linked `effect`,
    // and the kind decides what equipping it surfaces:
    //   bonus   → a live modifier on the Bonus ledger / Overview
    //   move    → a Move added to your Overview
    //   spell   → access to a spell, added to your grimoire
    //   ability → an untracked knack (e.g. casting silently) — narrative only,
    //             nothing to wire to the sheet; it just rides at the table.
    // The database this links to fills in the real mechanic; the shape below is
    // what the UI reads. `label` is always the human-readable summary.
    // Condition is tracked in MATERIALS: it runs from 0 to the wand's
    // maxCondition (its build cost), and starts full. The descriptions tell you
    // what spends it — most abilities chip 50/100/250 materials' worth at a time.
    // It is edited freely on the card, and a Wandcrafting roll (Repair) restores
    // it to full. `twisted: true` marks a forbidden wand — it draws a note.
    wands: [
    ...
    // ---- Glyphs (combine into a Rune, then roll Runology to create it) --
    glyphs: [
    ...
    // ---- Miscellaneous items (tracked, not load-bearing) ----------------
    items: [
